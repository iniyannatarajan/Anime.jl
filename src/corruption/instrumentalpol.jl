...
function instrumentalpol(obs::CjlObservation)
	...
	polrotmatrices = ones(elemtype, 2, 2, obs.numchan, size(uniqtimes)[1], size(obs.stationinfo)[1])
        ...
        for ant in 1:size(obs.stationinfo)[1]
            djonesmatrices[1, 2, :, ant] = gentimeseries(obs.yamlconf["instrumentalpol"]["mode"], obs.stationinfo.d_pol1_loc[ant], obs.stationinfo.d_pol1_scale[ant], obs.numchan, obs.rngcorrupt)
            djonesmatrices[2, 1, :, ant] = gentimeseries(obs.yamlconf["instrumentalpol"]["mode"], obs.stationinfo.d_pol2_loc[ant], obs.stationinfo.d_pol2_scale[ant], obs.numchan, obs.rngcorrupt)
